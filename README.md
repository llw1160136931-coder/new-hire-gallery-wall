...
课程思维导图仅支持 JPG、PNG、WebP，最大 10MB；课程资料仅支持 PDF、PPTX，单个最大 100MB、单次请求最大 200MB、每门课程最多 10 份。扩展名、图片真实格式、PDF 文件头和 PPTX 内部结构均由后端校验。课程文件不会写入公开的 `MEDIA_ROOT`，课程接口也不会返回磁盘路径。
