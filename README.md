...
- 学员可在三个固定时段使用 4 位签到码签到；管理员可生成签到码、查看签到数据，并为已结束场次补签或撤销管理员补签。
- 个人中心支持编辑姓名、头像、工作单位、MBTI、星座和性别。
- 学员可在个人中心查看和下载当前培训期内只属于自己的人才画像报告。
...
POST   /api/attendance/admin/makeups/ # 仅管理员，为已结束场次中的当前培训期学员补签
POST   /api/attendance/admin/makeups/{record_id}/revoke/ # 仅管理员，撤销管理员补签
...
GET    /api/me/talent-profile/   # 学员读取本人当前培训期的人才画像元数据
GET    /api/me/talent-profile/file/ # 学员鉴权读取本人画像文件
...
管理员可在签到管理页为当前培训期、已经结束且生成过签到码的场次补签，也可以撤销管理员补签；学员仍不能自行补签。补签与撤销均要求填写 5-200 字原因，操作人、真实操作时间和历史审计由后端记录，正常签到不能通过补签接口撤销。管理员补签接口默认限制为每个账号每分钟 30 次请求。学员必须属于当前培训期才能查看或提交签到，批量导入学员时会自动建立培训期归属。
...
人才画像按“培训期 + 学员账号”绑定，导入完成后不会再依赖学员可自行修改的姓名。原始 ZIP 不得提交到 Git；应临时上传至服务器，在应用代码和数据库迁移部署完成后先执行只读预演：

```bash
python manage.py migrate
python manage.py import_talent_profiles "/tmp/新员工人才画像生成.zip" --dry-run
```

预演会完整检查 ZIP 路径、HTML 编码和模板、报告正文姓名、重复姓名、受保护文件大小，以及报告集合是否与当前培训期的普通学员一一对应。默认情况下，任何缺失、多余、同名或无法唯一匹配的账号都会令整批导入停止，不会写数据库或文件。

如培训期中有明确不发布画像的备用账号或暂缓发布学员，可按档案姓名逐个显式声明；参数可重复使用：

```bash
python manage.py import_talent_profiles "/tmp/talent-profiles.zip" \
  --allow-missing-name "备用账号姓名" \
  --allow-missing-name "暂缓发布学员姓名" \
  --dry-run
```

每个例外姓名必须唯一对应所选培训期中的普通学员，并且确实不在 ZIP 中；空姓名、重复参数、错误姓名、ZIP 已包含的姓名，以及任何未显式声明的缺失学员仍会令整批导入停止。正式导入时必须使用与预演完全相同的 `--allow-missing-name` 参数。这些学员仍保留培训期成员身份，个人中心会显示画像暂未发布。

预演通过后再正式导入：

```bash
python manage.py import_talent_profiles "/tmp/新员工人才画像生成.zip"
```

如需导入非当前培训期，显式指定培训期 slug：

```bash
python manage.py import_talent_profiles "/tmp/talent-profiles.zip" --camp 2026-new-hires --dry-run
```

重复执行相同内容会按 SHA-256 跳过；如确实需要替换已发布且内容不同的报告，必须显式增加 `--replace-existing`，并先再次执行 `--dry-run`。导入后应立即安全删除服务器上的原始 ZIP。画像文件位于 `COURSE_MATERIAL_ROOT/talent-profiles/` 受保护目录，只能通过本人当前培训期的鉴权接口读取，不会生成公开的 `/media/` 地址。

在线查看和下载时，前端会读取当前登录学员在系统中的最新头像，校验为不超过 5MB 的受支持栅格图片后转换为 `data:` 内嵌图片，并替换画像模板中唯一的姓名首字占位；没有头像或读取失败时保留原占位并提示学员。个性化 HTML 会放入不具备同源权限的沙箱 iframe，并注入限制脚本、图片来源、网络和跳转能力的 CSP；后端始终以附件响应，避免画像 HTML 在系统同源页面直接执行。服务器保存的原始画像不会被改写，学员更新头像后再次查看或下载即可使用新头像。当前原始报告依赖固定版本的 jsDelivr Chart.js，下载后离线打开时图表可能无法显示。
...
课程资料和人才画像必须配置到独立的受保护目录，并通过 Nginx `internal` 路由发送。示例环境变量：
